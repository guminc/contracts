...
contract StolenNftFilterOracle is ReservoirOracle, Owned {
  ...
  constructor() Owned(msg.sender) ReservoirOracle(msg.sender) {}
  ...
  function validateTokensAreNotStolen(
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      require(
        _verifyMessage(expectedMessageId, validFor, message),
        "Message has invalid signature"
      );
      ...
      require(
        lastTransferTime + cooldownPeriod < block.timestamp,
        "NFT was transferred too recently"
      );
    }
  }
}
