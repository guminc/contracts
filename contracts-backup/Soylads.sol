...
abstract contract MiladyMaker {
  ...
  function ownerOf(uint256 idx) public view virtual returns (address);
}
...
contract Soylads is Ownable, ReentrancyGuard, ERC721A {
  ...
  function _computePrice(
    ...
    if ((numAlreadyMinted + numTokens) > RATWELL_CUTOFF) {
      ...
      return
        ((MINT_PRICE - IND * MILADY_DISCOUNT) * nUnder) +
        ((RATWELL_TAX_MINT_PRICE - IND * MILADY_DISCOUNT) * nAbove);
    }
